  ...
  <div class="container"
    ...
    <div class="listContainer">
      ...
      <div class="list" v-for="item in listData" @click="goDetail(item)" v-else>
        <div class="left">
          <p class="riverName">{{item.xmmc}}</p>
          <p class="second">
            <span>{{item.yzqybm}}</span>
            <span>——</span>
            <span>{{item.bwxz}}</span>
          </p>
          <p class="second">
...
<script>
  export default {
    ...
    methods: {
      ...
      getList() {
        this.postData('http://115.236.161.140:8090/wechat/getGkxm', {xmmc: this.input})
          .then(data => {
            this.loading = false
      ...
      goDetail(item){
        ...
        //   data:item
        // })
        this.$router.push(`/xmgcxq/${item.gkzjsxmId}`)
      }
    },
    created () {
      this.getList()
