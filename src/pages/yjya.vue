<template>
  <div class="yjyacontainer"
       v-loading="loading"
       element-loading-text="拼命加载中"
       element-loading-spinner="el-icon-loading"
       element-loading-background="rgba(0, 0, 0, 0.6)">
    <div class="yjyaheaderContainer">
    ...
    <div class="listContainer">
      ...
      <div class="list" v-for="item in listData" @click="goDetail(item)" v-else>
        <div class="left">
          <p class="riverName">{{item.yaName}}</p>
          <p class="second" v-if="item.filePath">
            <span>文件名称</span>
            <span>——</span>
            <span>{{item.filePath}}</span>
          </p>
          <p class="second">
            <span v-if="item.bzDate">编制时间</span>
            <span v-if="item.bzDate">——</span>
            <span>{{timestampToTime(item.bzDate)}}</span>
          </p>
        </div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getList() {
        this.postData('http://115.236.161.140:8090/wechat/getYjya', {yaName: this.input})
          .then(data => {
            this.loading = false
      ...
      goDetail(item){
        ...
        //   data:item
        // })
        this.$router.push(`/yjyaxq/${item.id}`)
      }
    },
    created () {
      this.getList()
