<template>
  ...
  >
    ...
    <div class="gkhblistContainer" v-for="(item,index) in listData" :key="item.id" v-if="index!==0" @click="goDetail(item)">
      <p class="gkhbtitle">{{item.newsTitle}}</p>
...
<script>
  export default {
    ...
    methods: {
      getList() {
        fetch('http://115.236.161.140:8090/wechat/getGkhbNews')
          .then((response) => {
