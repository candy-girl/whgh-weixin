<template>
  ...
  >
    ...
    <div class="gqdtlistContainer" v-for="(item,index) in listData" :key="item.id" v-if="index!==0" @click="goDetail(item)">
      <p class="gqdttitle">{{item.newsTitle}}</p>
...
<script>
  export default {
    ...
    methods: {
      getList() {
        fetch('http://115.236.161.140:8090/wechat/getGqdtNews')
          .then((response) => {
